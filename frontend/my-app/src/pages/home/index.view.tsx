...
export default function HomeView({ loading, error, items, onRetry }: Props) {
  ...
  if (error) return (
    <div className={styles.state}>
      <p>{error}</p>
      <button onClick={onRetry} className={styles.retryBtn}>다시 시도</button>
    </div>
  );
  if (!items.length) return <div className={styles.state}>아직 약속이 없어요.</div>;

  return (
    <div className={styles.wrap}>
      <h1 className={styles.title}>나의 약속</h1>
